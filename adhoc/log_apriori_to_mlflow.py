...
def log_apriori_artifacts(
    ...
    eval_metrics = model_payload.get("eval_metrics", {}) or {}
    ...
    with mlflow.start_run(run_name="apriori_training") as run:
        ...
        mlflow.log_param("eval_top_k", params.get("eval_top_k"))
        mlflow.log_param("eval_holdout_frac", params.get("eval_holdout_frac"))
        ...
        # Recommendation-quality metrics computed by evaluate_recommendations()
        # in train_model_apriori.py and stored in the pickle payload. Logging
        # them here too keeps this script's output consistent with the
        # in-pipeline logging, so monitoring.py finds recall_at_k (or
        # whichever --metric-key it's given) regardless of which script
        # produced the MLflow run.
        ...
        print(f"Logged eval metrics: {eval_metrics}")
